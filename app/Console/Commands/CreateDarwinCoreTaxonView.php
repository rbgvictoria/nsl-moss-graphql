...
class CreateDarwinCoreTaxonView extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'dwc:create:taxon-view';
     ...
     * @var string
     */
    protected $description = 'Creates materialized view with Darwin Core Taxon Core data';
    ...
    {
        $sql = <<<EOT
CREATE MATERIALIZED VIEW dwc_taxon AS
...
EOT;
        $this->info('Drop existing view...');
        DB::statement('DROP MATERIALIZED VIEW dwc_taxon');

        DB::statement($sql);
        $this->info('View public.dwc_taxon has been created.');
